	...
	function toggleField(topic: string, field: string, multiId: number) {
		activePlots.update((plots) => {
			...
			if (existing) {
				...
				return [newPlot, ...plots];
			}
		});
	}

	function close() {
		builderOpen.set(false);
	}
</script>
...
{#if $builderOpen}
	...
	<div class="fixed inset-0 z-40">
		...
		<!-- Panel -->
		<div class="fixed inset-y-0 right-0 z-50 w-full max-w-xs sm:max-w-sm md:max-w-80 overflow-y-auto bg-white shadow-xl">
			<!-- Header -->
			<div class="flex items-center justify-between border-b border-gray-200 px-4 py-3">
				<h2 class="text-sm font-semibold text-gray-900">Edit Plots</h2>
